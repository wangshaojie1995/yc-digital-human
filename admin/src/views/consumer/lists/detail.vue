<template>
    <div>
            ...
            <el-form ref="formRef" class="ls-form" :model="formData" label-width="120px">
                <div class="bg-page flex py-5 mb-10 items-center">
                    ...
                    <div class="basis-40 flex flex-col justify-center items-center">
                        ...
                        <div class="mt-2 flex items-center">
                            {{ formData.points }}
                            <el-button v-perms="['user.user/adjustPoints']" type="primary" link
                                @click="handleAdjustPoints(formData.points)">
                                调整
                            </el-button>
                        </div>
                    </div>
        ...
        <account-adjust-points v-model:show="adjustState.showPoints" :value="adjustState.value"
            @confirm="handleConfirmAdjustPoints" />
    </div>
</template>
...
import { adjustMoney, adjustPoints, getUserDetail, userEdit } from '@/api/consumer'
import { isEmpty } from '@/utils/util'

import AccountAdjust from '../components/account-adjust.vue'
import AccountAdjustPoints from '../components/account-adjust-points.vue'
...
const handleAdjustPoints = (value: string) => {
    adjustState.showPoints = true
    adjustState.value = value
}
...
const handleConfirmAdjustPoints = async (value: any) => {
    await adjustPoints({ user_id: route.query.id, ...value })
    adjustState.showPoints = false
    getDetails()
}
