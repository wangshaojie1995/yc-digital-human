...
class AccountLogEnum
{
     ...
     * 用户点数减少类型
     */
    const PM_DEC_ADMIN = 300;
    const PM_DEC_SCENE_CREATE = 301;
    const PM_DEC_VOICE_CREATE = 302;
    const PM_DEC_VIDEO_CREATE = 303;


    /**
     * 用户点数增加类型
     */
    const PM_INC_ADMIN = 400;
    const PM_INC_RECHARGE = 401;
    const PM_INC_CDKEY = 402;


    const PM_DEC = [
        self::PM_DEC_ADMIN,
        self::PM_DEC_SCENE_CREATE,
        self::PM_DEC_VOICE_CREATE,
        self::PM_DEC_VIDEO_CREATE,
    ];

    const PM_INC = [
        self::PM_INC_ADMIN,
        self::PM_INC_RECHARGE,
        self::PM_INC_CDKEY,
    ];
    ...
    public static function getChangeTypeDesc($changeType, $flag = false)
    {
        $desc = [
            ...
            self::PM_DEC_ADMIN => '平台减少点数',
            self::PM_INC_ADMIN => '平台增加点数',
            self::PM_INC_RECHARGE => '充值增加点数',
            self::PM_INC_CDKEY => '卡密兑换增加点数',
            self::PM_DEC_SCENE_CREATE => '场景复刻减少点数',
            self::PM_DEC_VOICE_CREATE => '语音合成减少点数',
            self::PM_DEC_VIDEO_CREATE => '视频合成减少点数',
    ...
    {
        return array_merge(self::UM_DEC, self::UM_INC, self::PM_DEC, self::PM_INC);
    }
    ...
    {
        return array_merge(self::PM_DEC, self::PM_INC);
    }
